...
@Component({
  selector: 'app-editar-suplente',
  templateUrl: './editar-suplente.component.html',
  styleUrls: ['./editar-suplente.component.css']
})
export class EditarSuplenteComponent implements OnInit {
  dniJugador : string = '';
  blockSpace: RegExp = /[^\s]/;
  ...
  editarSuplente(): void {
    let suplente = {...this.jugadorPartidoDto};
    suplente.idJugador = this.jugador.id!;
    this.jugadorPartidoService.editarParticipacion(suplente, this.participacionTitular.id).toPromise().then(
      data => {
